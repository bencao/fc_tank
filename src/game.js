...
export class Game {
  constructor() {
    this.canvas   = new Kinetic.Stage({container: 'canvas', width: 600, height: 520});
    ...
    this.scenes   = {
      ...
      'battle_field': new BattleFieldScene(this, new BattleFieldView(this.canvas)),
      'report'      : new ReportScene(this, new ReportView(this.canvas))
    };
    this.current_scene = null;
  }

  get_config(key) { return this.configs[key]; }

  update_status(key, value) { return this.statuses[key] = value; }

  get_status(key) { return this.statuses[key]; }
  ...
  kick_off() { return this.switch_scene('welcome'); }

  prev_stage() {
    return this.statuses['current_stage'] = this.mod_stage(this.get_status('current_stage'), -1);
  }
  ...
  switch_scene(type) {
    ...
    if (!_.isEmpty(this.current_scene)) { this.current_scene.stop(); }
    target_scene.start();
